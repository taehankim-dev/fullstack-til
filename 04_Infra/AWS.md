...
## 8. 관계형 데이터베이스 설정 (Amazon RDS)
- **엔진**: MySQL 8.0 (Free Tier)
- **초기 DB 설정**: `CREATE DATABASE portfolio;` 명령을 통해 수동 생성 및 스키마 확보.
- **연동 전략**:
  - 정적 파일(MDX)은 S3에 보관하고, 메타데이터는 RDS에서 관리하는 구조로 설계.
  - 외부 접근을 차단하고 오직 EC2(Internal)에서만 접근 가능한 보안 계층 구축.

## 9. 보안 그룹 체인
- **설정**: 특정 IP 주소가 아닌, 보안 그룹 ID를 기반으로 접근 권한 부여.
- **RDS 인바운드 규칙**:
  - EC2 전용 보안 그룹 ID를 입력하여 해당 EC2 서버만 DB에 접속할 수 있도록 제한.
